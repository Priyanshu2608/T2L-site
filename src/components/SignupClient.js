...
export default function SignupClient() {
  ...
  const handleSocialClick = (socName) => {
    triggerToast(`${socName} sign-up is coming soon.`);
  };
  ...
  const handleFormSubmit = (e) => {
    ...
    // Consent validation
    if (!consent) {
      setConsentErr("Please accept the Terms and Privacy Policy.");
      hasError = true;
    } else {
      setConsentErr("");
    }
    ...
    setTimeout(() => {
      setShowSuccess(true);
      setTimeout(() => {
        // If there was a prompt query, we could redirect to `/docengine-app` or similar, 
        // but since we are replicating, taking them back home or to docengine is highly appropriate.
        router.push("/");
      }, 1100);
    }, 700);
  };
      ...
      <main className="auth">
        ...
        <section className="brandside" aria-hidden="true">
          ...
          <div className="brand-head">
            <h1>Let's get you<br /><span class="g">started.</span></h1>
            <p>Create your account and unlock the full power of Turn2Law.</p>
          </div>
        ...
        <section className="formside">
          <div className="card">
            ...
            <p className="sub">Join Turn2Law — India's Legal OS for founders and lawyers.</p>
            
            {promptQuery && (
              <div style={{ padding: "12px", background: "var(--gold-soft)", borderRadius: "10px", marginBottom: "16px", fontSize: "13px", border: "1px solid rgba(216,171,91,.25)" }}>
                Prefilling draft session from Doc Engine: <b style={{ color: "var(--ink)" }}>"{promptQuery}"</b>
              </div>
            )}
            ...
            <form onSubmit={handleFormSubmit} noValidate>
              <div className="field">
                ...
                <small className={`ferr ${nameErr ? "show" : ""}`} id="nameErr" role="alert">{nameErr}</small>
              </div>
              ...
              <small className={`ferr ${consentErr ? "show" : ""}`} id="consentErr" role="alert">{consentErr}</small>
              ...
              <div className="formok" id="formOk" style={{ display: showSuccess ? "flex" : "none" }}>
                ...
                <span>Account created. Taking you in…</span>
              </div>
            </form>
            ...
            <p className="alt">Already have an account? <Link href="/login">Log in</Link></p>
          </div>
        </section>
      </main>
